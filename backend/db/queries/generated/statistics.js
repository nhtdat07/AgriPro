/**
 * Executes the 'getTotalBenefit' query.
 * @param {Object} params - Parameters for the query.
 * @returns {Promise<Array>} - Query result rows.
 */
export async function getTotalBenefit(pool, params = {}) {
    try {
        const query = `SELECT SUM((invoice_product.price - inventory_product.in_price) * invoice_product.quantity)::INTEGER AS total_benefit
...
/**
 * Executes the 'getStatisticsPurchasing' query.
 * @param {Object} params - Parameters for the query.
 * @returns {Promise<Array>} - Query result rows.
 */
export async function getStatisticsPurchasing(pool, params = {}) {
    try {
        const query = `SELECT COUNT(*)::INTEGER AS order_quantity, SUM(total_payment)::INTEGER AS total_purchasing
FROM purchase_order
...
/**
 * Executes the 'getStatisticsSelling' query.
 * @param {Object} params - Parameters for the query.
 * @returns {Promise<Array>} - Query result rows.
 */
export async function getStatisticsSelling(pool, params = {}) {
    try {
        const query = `SELECT COUNT(*)::INTEGER AS invoice_quantity, SUM(total_payment)::INTEGER AS total_selling
FROM sales_invoice
WHERE agency_id = $1 AND recorded_at >= $2::DATE AND recorded_at < $3::DATE + INTERVAL '1 day';`;
        const { rows } = await pool.query(query, Object.values(params));
        return rows;
...
/**
 * Executes the 'getSoldProducts' query.
 * @param {Object} params - Parameters for the query.
 * @returns {Promise<Array>} - Query result rows.
 */
export async function getSoldProducts(pool, params = {}) {
    try {
        const query = `SELECT invoice_product.product_id AS product_id, product.name AS product_name, 
